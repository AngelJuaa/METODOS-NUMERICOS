...
public class UnidadIIIServiceImpl implements UnidadIIIService {
    ...
    public ReglaCramer AlgoritmoReglaCramer(ReglaCramer modelCramer) {
        // Almacenamos determinantes de la matriz
        ...
        switch (modelCramer.getMN()) {
            ...
            case 3 -> {
                ArrayList<Double> A = modelCramer.getMatrizA();
                ArrayList<Double> B = modelCramer.getMatrizB();
                ...
                // Calculamos determinante para x3
                double[][] matrizX3 = {
    ...
    public Jacobi AlgoritmoJacobi(Jacobi modelJacobi) {
        ...
        ArrayList<Double> x0 = modelJacobi.getX0();
        ArrayList<Double> x = new ArrayList<>();

        int maxIter = modelJacobi.getIteraciones();
        double tol = modelJacobi.getTolerancia();

        for (int i = 0; i < n; i++) {
            x.add(0.0); // inicializamos vector solución
        }

        for (int k = 0; k < maxIter; k++) {
            ArrayList<Double> nuevoX = new ArrayList<>();
            for (int i = 0; i < n; i++) {
                double suma = 0.0;
                for (int j = 0; j < n; j++) {
                    if (i != j) {
                        suma += A.get(i * n + j) * x0.get(j);
                    }
                }
                nuevoX.add((B.get(i) - suma) / A.get(i * n + i));
            }

            // Calcular error
            double error = 0.0;
            for (int i = 0; i < n; i++) {
                error += Math.abs(nuevoX.get(i) - x0.get(i));
            }

            x0 = new ArrayList<>(nuevoX);
